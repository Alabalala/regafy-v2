...
export async function checkRequestStatus(currentUserId, targetUserId, supabase) {
...
// GIFTS
export async function getGifts(userId, supabase) {
...
export async function updateGift(formData, supabase) {
  const { name, price, comments, shop_link: shopLink, size, id, image_link } = formData
...
export async function createGroup(formData, userId, supabase) {
  ...
  const { error, data } = await supabase
    .from("groups")
    .insert({ name: name, description: description, "cover_photo": coverPhoto, creator: userId })
    .select("*");
